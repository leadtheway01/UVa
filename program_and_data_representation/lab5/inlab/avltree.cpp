...
{
    // YOUR CODE HERE
    double expected_path = int_path_length(root, 0) / card_of();
    return expected_path;  // stub, remove after writing your code
}
...
AvlTree::find( const string & x, AvlNode *t ) const {
    while ( t != NULL )
        if ( x < t->element ) {
            t = t->left;
            LeftLinksFollowed++;        //to count left link followed
        } else if ( t->element < x ) {
            t = t->right;
            RightLinksFollowed++;       //to count right link followed
        } else
            return t;    // Match
...
void AvlTree::rotateWithLeftChild( AvlNode * & k2 ) const {
    ...
    k2 = k1;
    SingleRotations++;  //increment single rotation counter 
}
...
//single left rotation on child, then single right rotation on parent
void AvlTree::doubleWithLeftChild( AvlNode * & k3 ) const {
    rotateWithRightChild( k3->left );
    rotateWithLeftChild( k3 );
    DoubleRotations++;  //increment double rotation counter
}
...
//single right rotation on child, then single right rotation on parent
